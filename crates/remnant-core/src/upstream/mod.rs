...
impl UpstreamFetcher {
    ...
    /// `package_name` must already be validated by the caller (e.g. the proxy's
    /// `ValidatedPackageName::as_str()`, or a future CLI-side equivalent) before this
    /// is called — this crate does not validate npm package names itself; that
    /// responsibility stays with each ecosystem-specific consumer per
