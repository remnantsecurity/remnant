use super::{ConfigError, validate_proxy_config};
...
fn rejects_empty_origin_as_not_https() {
    let result = validate_proxy_config("".into(), "127.0.0.1:4873".into(), false);
...
fn rejects_ftp_origin_without_insecure_flag() {
    let result = validate_proxy_config("ftp://example.com".into(), "127.0.0.1:4873".into(), false);
